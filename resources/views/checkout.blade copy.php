            ...
            <div class="container">
                <div class="row">
                                            ...
                                            <div class="form-row">
                                                ...
                                                <div class="from-group col-md-6 mb-3">
                                                    ...
                                                    <div class="input-group">
                                                        ...
                                                        <div class="invalid-feedback"></div>
                                                        <div id="loading-address"
                                                            style=" right:0;margin-top:4px;margin-right:4px;display:none "
                                                            class="spinner-border  text-success position-absolute "
                                                            role="status">
                                                            <span class="sr-only">Chargement...</span>
                                                        </div>
                                                        <div class="position-absolute w-100 mt-5  border"
                                                            id="list_address_container" style="z-index:100;display:none">
                                                            <div class="list-group  " id="list_address">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                    ...
                    <div class="col-12 col-xl-3">
                        ...
                        <input type="button" class="swipe-to-top btn-block btn btn-danger" id="removeCoupon"
                            value="Remove Coupon" />
                    </div>
                </div>
            </div>
...
@section('script')
    ...
    <script>
        ...
        var ignoreClickOnMeElement = document.getElementById('list_address_container');

        document.addEventListener('click', function(event) {
            var isClickInsideElement = ignoreClickOnMeElement.contains(event.target);
            if (!isClickInsideElement) {
                $("#list_address_container").hide();
            }
        });
        $(document).ready(function() {
            $("#delivery_postcode").on("keyup", function() {
                var params = "";
                console.log($("#delivery_postcode").val());
                if ($.isNumeric($("#delivery_postcode").val())) {
                    params = "codePostal"
                } else {
                    params = "nom"
                }
                    ...
                    success: function(data) {
                        /* var result = {};
                         $.each(data.predictions, function(i, item) {
                             result[item.place_id] = item.description;
                         });
                         // alert(result);
                         //alert(data.predictions[0].place_id)
                         response(result);*/
                        $("#list_address").empty();
                        data.forEach(function(item) {
                            $("#list_address").append(
                                '  <a role="button" onclick="selectAddress(' + item
                                .code +
                                ')" class="list-group-item list-group-item-action" aria-current="true">' +
                                item.nom.toUpperCase() + " (" + item
                                .codesPostaux[0] + ")" + ' </a>'
                            )
                        });
                        if (data.length) {
                            $("#list_address_container").show();
                        }

                        $("#loading-address").hide();
                        /*response($.map(data, function(item) {

                            $('#autocomplete-load').attr('hidden', true);

                            return {
                                'label': item.nom.toUpperCase() + " (" + item
                                    .codesPostaux[0] + ")",
                                'value': item.code
                            };
                        }));*/
        ...
        function selectAddress(value) {
            ...
            $.ajax({
                url: "https://geo.api.gouv.fr/communes/" + value +
                ...
                success: function(data) {
                    $("#delivery_city").val(data.nom);
                    $("#delivery_postcode").val(data.code);
                    $("#delivery_state").val(data.departement.nom);
                    $("#list_address_container").hide();
                    // code block

                    /*$("input[name=lng]").val(data.centre.coordinates[0]);
                    $("input[name=lat]").val(data.centre.coordinates[1]);
                    $("input[name=placeId]").val(ui.item.value);
    ...
    <script>
        ...
        $("#defaultCheck").click(function() {
            if ($.trim($(this).prop('checked')) == 'true') {
                ...
                $("#billing_country").val($("#delivery_country").val());
                countries();
        ...
        $(".createOrder").click(function(e) {
            ...
            if (payment_method == 'razorpay') {
                ...
                var KeyId =
                    "{{ isset(PaymentMethods('kEY_ID')->value) ? PaymentMethods('kEY_ID')->value : '' }}";
