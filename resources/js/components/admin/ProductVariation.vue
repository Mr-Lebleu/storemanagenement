                                ...
                                <div class="card-body">
                                    <div>
                                        <div class=" table-responsive" id="printableTable">
                                            ...
                                            <div class="dataTables_length" id="productvariationTable_length"><label>Afficher 
                                            ...
                                            <div id="productvariationTable_filter" class="dataTables_filter">
                                                ...
                                                <ul class="pagination pagination-sm m-0 float-right">
                                                    <li v-bind:class="[{disabled: !pagination.prev_page_url}]"><button class="page-link" href="#" @click="fetchvariations(pagination.prev_page_url)">Précédent</button></li>

                                                    <li class="disabled"><button class="page-link text-dark" href="#">Page {{ pagination.current_page }} of {{ pagination.last_page }}</button></li>

                                                    <li v-bind:class="[{disabled: !pagination.next_page_url}]" class="page-item"><button class="page-link" href="#" @click="fetchvariations(pagination.next_page_url)">Suivant</button></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
        ...
        deletevariation(id) {
            if (confirm('Are You Sure?')) {
                ...
                axios.delete(`/api/admin/variation/${id}`,this.token)
                    .then(res => {
                        if (res.data.status == "Success") {
                            this.$toaster.success('Variante de produithas been Delete successfully')
        ...
        addUpdatevariation() {
            ...
            axios[this.request_method](url, this.variation, this.token)
                .then(res => {
                    if (res.data.status == "Success") {
                        this.display_form = 0;
                        this.$toaster.success('Variante de produithas been updated successfully')
                        this.clearForm();
                        this.fetchvariations();
                    } else {
